...
let viewRepresentable = BlogPost(metadata: .init(headline: "Preventing Runtime Warnings and Loops", title: "Working With UIViewRepresentable", date: "2023-09-13", updatedAt: "2023-09-15"), body: .pieces(myPostBody), link: "/post/view-representable")

extension String {
    func asBlock(color: Color = .gray) -> some View {
        Text(self)
            .padding(7)
            .background {
                ZStack {
                    let r = RoundedRectangle(cornerRadius: 5, style: .continuous)
                    r
                        .opacity(0.1)
                    r.stroke(lineWidth: 1)
                }
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            .background {
                RoundedRectangle(cornerRadius: 5, style: .continuous)
                    .stroke(Color.secondary, style: .init(dash: [5]))
            }
        }
        .frame(width: 600)
    }
...
let mainQueue0 = MainQueue(items: ["setSelection(new)"])
let mainQueue1 = MainQueue(items: ["setSelection(new)", "setSelection(old)"])

let mainQueue10 = MainQueue(items: ["setSelection…"])
let mainQueue11 = MainQueue(items: ["setSelection…", "setText…"])
...
fileprivate var myPostBody: [any PostPiece] {
    ...
    **Updating an NSView in Response to a SwiftUI State Change**
    ...
    """)
    SwiftUIView(background: false) { mainQueue11 }
    Markdown("""
    ...
    """)
    SwiftUIView(background: false) {
        MainQueue(active: "setSelection…", items: ["setText…"])
    }
    Markdown("""
    ...
    """)
    SwiftUIView(background: false) {
        MainQueue(active: "setSelection…", items: ["setText…", "updateView…"])
    }
    Markdown("""
    After the `setSelection` block is done, the `setText` is removed from the queue:
    """)
    SwiftUIView(background: false) {
        MainQueue(active: "setText…", items: ["updateView…"])
    }
    Markdown("""
